...
/**
 * Slideshow page for full-screen auto-rotating entry display.
 *
 * Perfect for party screens or event displays.
 * Features auto-advance, keyboard controls, and fullscreen mode.
 */
import {
  Play,
  Pause,
...
const { t } = useI18n()
const { entries, fetchEntries } = useGuests()
const {
  currentIndex,
  currentEntry,
  totalEntries,
...
/**
 * Shows controls and sets auto-hide timer.
 */
...
async function toggleFullscreen(): Promise<void> {
  ...
  } else if (slideshowEl.value) {
    await enterFullscreen(slideshowEl.value)
  }
}

/**
 * Updates fullscreen state based on document state.
 */
...
<template>
  ...
  >
      ...
      >
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/60 to-transparent pointer-events-auto">
          <div class="flex items-center justify-center gap-4">
            <!-- Previous -->
            <button
              class="rounded-full p-3 text-white/80 hover:bg-white/10 hover:text-white transition-colors"
              @click="prev"
            >
            ...
            <!-- Play/Pause -->
            <button
              class="rounded-full bg-white/20 p-4 text-white hover:bg-white/30 transition-colors"
              @click="toggle"
            >
    ...
    <Transition name="slide-up">
      <div
        v-if="showSettings"
        class="absolute bottom-24 left-1/2 -translate-x-1/2 rounded-xl bg-black/80 p-4 backdrop-blur-sm"
      >
