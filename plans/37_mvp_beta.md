...
- [x] Create `app/plugins/csrf.client.ts` — fetch CSRF token on app init and attach to all mutating requests
- [x] Ensure all `$fetch` POST/PUT/DELETE calls include the CSRF token header
- [x] Verify CSRF protection works end-to-end (login, register, profile, entry creation, admin actions)
...
**Status**: Completed prior to Plan 37 implementation.

### 37.2 2FA Frontend — Login Verification (from Plan 35.35) ✔

- [x] Add 2FA verification step to the login flow in `app/pages/login.vue`
- [x] After successful password auth, detect `requiresTwoFactor` response
- [x] Show code input (6-digit TOTP or Email OTP)
- [x] Support backup code fallback input
- [x] Call `POST /api/auth/2fa/verify` with the code + temp token
- [x] On success, proceed to dashboard; on failure, show error
...
- [x] Create `app/pages/profile/security.vue` or add inline setup to profile page
- [x] TOTP setup: Display QR code (from `POST /api/auth/2fa/setup`), verify with code
- [x] Email OTP setup: Trigger email, verify with code
- [x] Display backup codes after successful setup (one-time view, download option)
- [x] Disable 2FA option with password confirmation
- [x] Link from profile security section to setup flow

**Why**: Backend 2FA is fully implemented but there's no way for users to enable it without a frontend UI.

### 37.4 Error Pages ✔

- [x] Create `app/error.vue` — custom error page (404, 500, etc.)
- [x] Friendly error message with navigation back to home
- [x] Consistent with glassmorphism design system
...
- [x] Implement actual email sending (currently `email-otp.ts` logs to console)
- [x] Choose provider: Resend, Postmark, or SMTP via nodemailer
- [x] Environment variable for email configuration (`EMAIL_FROM`, `SMTP_*` or `RESEND_API_KEY`)
- [x] Email templates: 2FA OTP code, team invite, password reset (future)
- [x] Graceful fallback: log to console in development, fail loudly in production if not configured
...
- [x] Create `vercel.json` with Nuxt preset configuration
...
- [x] Configure vitest for Nuxt (`vitest.config.ts`)
- [x] Write unit tests for critical server utilities:
  - `server/utils/sanitize.ts` (17 tests)
  - `server/utils/rate-limit.ts` (5 tests)
...
- [x] Ensure all dev-only defaults are rejected in production:
...
- [x] Add startup validation: refuse to start in production with insecure defaults
...
- [x] Abstract photo/avatar storage behind a storage interface
...
- [x] Ensure all async data fetches show loading indicators
- [x] Add empty states for lists with no data (no guestbooks, no entries, no members)

**Why**: Currently many pages show blank content during data loading, which feels broken.

### 37.11 Form Validation Feedback ✔

- [x] Add inline validation errors to registration form (password policy feedback)
- [ ] Add inline validation errors to profile edit forms
- [x] Show password strength indicator on registration
...
- [x] Verify UserMenu dropdown works correctly on mobile (touch targets, positioning)
- [x] Ensure profile page is fully responsive on small screens
- [x] Test guestbook admin settings dialog on mobile
...
- [x] All P0 items completed and verified
- [x] All P1 items completed or explicitly deferred with documented workarounds
- [x] `pnpm build` passes
...
- [x] Security: CSRF, 2FA, rate limiting, password policy all functional end-to-end
- [x] No insecure defaults in production
- [x] Basic test coverage for critical paths (43 tests across 5 suites)
