/**
 * DELETE /api/tenants/:uuid/guestbooks/:gbUuid/share-image
 * Removes the Open Graph share image for a guestbook.
 * Deletes the file from disk and clears shareImageUrl from settings.
 */
...
export default defineEventHandler(async (event) => {
  ...
  // Delete all share.* files from disk
  ...
  // Clear shareImageUrl from settings
  const settings = { ...(existing.settings || {}) }
  delete settings.shareImageUrl
  const updated = await updateGuestbook(gbUuid, { settings })

  await recordAuditLog(event, 'guestbook.shareImage.delete', {
    tenantId: uuid,
