/**
 * POST /api/tenants/:uuid/guestbooks/:gbUuid/share-image
 * Uploads a dedicated Open Graph share image for a guestbook.
 * Accepts multipart/form-data with an image file (max 5MB).
 * Encrypts and stores the image, updates guestbook settings (shareImageUrl).
 */
...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    const settings = { ...(existing.settings || {}), shareImageUrl: cacheBustedUrl }
    const updated = await updateGuestbook(gbUuid, { settings })

    await recordAuditLog(event, 'guestbook.shareImage.upload', {
      tenantId: uuid,
